...
* Eine neue empfohlene Aktion zum Sichern geschäftskritischer Dateien in Datenquellen zeigt, wie Sie Ihre wichtigsten Datenkategorien durch das Sichern von Volumes mit Cloud Backup schützen können. Dies ist wichtig für den Fall, dass Sie aufgrund eines Ransomware-Angriffs alle Daten wiederherstellen müssen. Die Empfehlung leitet Sie zur Cloud Backup-Schnittstelle um, damit Sie Backups auf den erforderlichen Volumes ermöglichen können.
